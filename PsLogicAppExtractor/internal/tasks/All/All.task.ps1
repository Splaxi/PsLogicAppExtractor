...
#Original file: Set-Arm.Connections.ManagedApis.AmazonS3.AsArmObject.task.ps1
$parm = @{
    Description = @"
...
"@
    Alias       = "Arm.Set-Arm.Connections.ManagedApis.AmazonS3.AsArmObject"
}

Task -Name "Set-Arm.Connections.ManagedApis.AmazonS3.AsArmObject" @parm -Action {
    Set-TaskWorkDirectory
    ...
    foreach ($connectionObj in $armObj.resources[0].properties.parameters.'$connections'.value.PsObject.Properties) {
        if ($connectionObj.Value.id -like "*managedApis/amazons3*"`
            ...
            $apiObj = Get-Content -Path "$pathArms\API.Amazon.S3.json" -Raw | ConvertFrom-Json
